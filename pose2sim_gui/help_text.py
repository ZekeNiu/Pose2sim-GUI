...
HELP_TEXTS: dict[str, str] = {
    ...
    "wizard_calibration_route": "选择本次项目的校准路径。已有 Calib.toml 时可直接放入 calibration/ 并跳过校准；普通手机新项目通常选择“从素材计算标定”；如果暂时没有标定，只能做 2D 姿态识别和同步质量检查，不能做可信的三维重建或 OpenSim 关节角。",
    "wizard_extrinsics_extension": "外参素材文件扩展名，例如 mp4、jpg、png。scene 方法可使用每台相机看到场景点的短视频或图片；board 方法使用固定位置的棋盘格素材。",
    "wizard_intrinsics_corners": "棋盘格内角点数，不是棋盘格总方格数。例如棋盘格横向有 9 格、纵向有 6 格，则内角点通常是 8, 5。必须与实际打印板一致。",
    "wizard_intrinsics_square": "棋盘格每个方格的实际边长，单位为毫米。请用尺测量打印后或购买的标定板，不要只使用打印文件的理论尺寸。",
